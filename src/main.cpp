/*
 * Patrons de conception: Composant et State
 * // COMMENTAIRES NE SONT PAS TERMINÉS
 */
...
static const std::string AppTitle = "Patrons de conception: Composant et State";
...
class GameApp final /* La classe est final, aucune classe dérivée ne peut être créée. */
{
  ...
  public:
	...
	GameApp ()
	{
		...
		Entities.push_back (new Character_Rat ());
		Entities.push_back (new Character_Rat ());
		Entities.push_back (new Character_Rat ());
		Entities.push_back (new Character_Punk ());
		Entities.push_back (new Character_Punk ());
		Entities.push_back (new Character_Punk ());
	}
		...
		while (running == true)
			{
				...
				if (keyboard[SDL_SCANCODE_W] == true)
					{
						player->movement.velocity.y = -50.f;
					}
				...
				if (keyboard[SDL_SCANCODE_LEFT] == true)
					{
						dynamic_cast<Player *> (player)->Shoot (Entities, (SDL_Point){ -1, 0 });
					}
